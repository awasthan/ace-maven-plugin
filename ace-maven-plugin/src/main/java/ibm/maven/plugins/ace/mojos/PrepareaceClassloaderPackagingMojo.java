...
public class PrepareaceClassloaderPackagingMojo extends AbstractMojo {
     ...
     */
    @Parameter(property = "ace.classloaderPath", defaultValue = "${project.build.directory}/ace/classloader", required = true, readonly = true)
    protected File classloaderPath;

    public void execute() throws MojoExecutionException, MojoFailureException {
        // mvn
        // org.apache.maven.plugins:maven-dependency-plugin:2.1:copy-dependencies
        // -DoutputDirectory=${project.build.directory}/ace/classloader

        getLog().info("Emptying " + new File(project.getBuild().getDirectory(), "ace").getAbsolutePath());
        try {
            FileUtils.deleteDirectory(new File(project.getBuild().getDirectory(), "ace"));
        } catch (IOException e) {
            // ignore
        }
